    ...
    mainPanel(
      ...
      tabsetPanel(
        ...
        tabPanel("Top-5 hits",
                 ...
                 tags$ul(
                   tags$li(strong("Cluster:"), "The name or number of the unknown cluster"),
                   tags$li(strong("Ref_cell_type:"), "Broad classification of the reference cell type"),
                   ...
                   tags$li(strong("Reference_score_sum:"), "Aggregate identity score for the given reference cell type"),
                   tags$li(strong("Index:"), "Position in the dataframe (needed for ordered plotting)"),
                   tags$li(strong("Z-score:"), "How many standard deviations apart is the reference score sum from the average score for that cluster (see How-to tab for details)"),
                   tags$li(strong("Percent_pos_correlation:"), "Frequency of genes that exhibit positive correlation between the unknown cluster and a given reference cell type (see How-to tab for details)")
                 )
                 )
        ),
        ...
        tabPanel("How to use this program",
                 ...
                 h3("Common problems"),
                 p("You may get an error message and disconnect from the server if the uploaded data file is not formatted properly. Some common issues include:"),
                 tags$ul(
                   tags$li(strong("Incorrectly naming the columns."), "Our algorithm should be agnostic to capitalization, but the column names should include", strong(em("Gene, Cluster, and logFC.")), "The data frame generated by Seurat's FindAllMarkers() function can work directly in our algorithm."),
                   tags$li(strong("Using data from other species (unless a separate reference file is provided)."), "If you are using pre-loaded ImmGen datasets for comparisons, your cluster gene signature file should contain mouse genes. If your data originate from human studies, you can use online tools to convert human genes to their mouse homologs. For most gene names, mouse homologs can be obtained by converting capitalized gene name into sentence case names (e.g. PDCD1 to Pdcd1). Excel's PROPER() function is handy for such conversions.")
                 ),
